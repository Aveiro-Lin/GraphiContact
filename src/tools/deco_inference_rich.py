...
from src.utils.renderer import Renderer, visualize_reconstruction_and_att_local, visualize_reconstruction_no_text, visualize_reconstruction_no_text_new
    ...
    with torch.no_grad():
        for image_file in image_list:
            if 'pred' not in image_file:
                ...
                # forward-pass
                pred_camera, pred_3d_joints, pred_vertices_sub2, pred_vertices_sub, pred_vertices, hidden_states, att, pred_contact_ver, _ = Graphormer_model(batch_imgs, smpl, mesh_sampler)
                ...
                label = torch.where(pred_contact_ver>0.55, 1, 0)
                #print(label.size())
                ...
                # visual_imgs_output = visualize_mesh( renderer, visual_imgs_output,
                #                                                 pred_label[0].detach(), 
                #                                                 pred_camera1.detach(),
                #                                                 color='light_blue')
...
###########################################################################################
                # The following part is newly added and is projected once on the original basis
                # Use the newly generated 3D projected image as the backdrop is being projected
                new_background_img = Image.open(temp_fname)
...
def parse_args():
    ...
    # There is no need to call the test.npz file, there is no gt_ana so there is no way to calculate precisionrecallf1fp errorfn error
    ...
    parser.add_argument("--deco_resume_checkpoint", default='Path/to/GraphiContact/ckpt/deco_grph_rich.pt', type=str, required=False,
                        help="Path to specific checkpoint for resume training.")
