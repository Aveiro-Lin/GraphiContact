...
from src.modeling.bert import Graphormer_Body_Network_SIMU as Graphormer_Network_SIMU
...
def main(args):
    ...
    for i in range(100):
        ...
        for idx, batch in enumerate(train_data):
            ...
            #     org_img_arry = [transform(Image.open(image_file).convert('RGB')) for image_file in org_img_batch]
...
def train_batch(model, smpl, mesh_sampler, batch, optimizer, scheduler, loss_fn, dev='cuda', n_infers=1):
    ...
    if len(org_img_batchs[0]) >= 1:
        ...
        org_img_arry = [[transform(Image.open(image_file).convert('RGB')) for image_file in org_img_batch] for org_img_batch in org_img_batchs]
...
def prepare_dataset(batch_size, total_num=4384, data_path='HOT-Annotated/images',
                    label_path='HOT-Annotated/Release_Datasets/damon/hot_dca_trainval.npz',
                    n_infers=3):
    total_num = total_num
...
def load_models(args):
    ...
    _model = Graphormer_Network_SIMU(args, config, backbone, trans_encoder, trans_encoder2, mesh_sampler)
