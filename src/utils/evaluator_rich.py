...
def evaluator(contact_labels_3d_pred, contact_labels_3d_gt, normalize=True, return_dict=False):
    ...
    return cont_pre.mean(), cont_rec.mean(), cont_f1.mean(),  fn_geo_err.mean()*5000, fp_geo_err.mean()*5000
...
class Evaluator:
    ...
    def __call__(self, contact_labels_3d_pred, contact_labels_3d_gt, normalize=True, return_dict=False):
        ...
        return cont_pre.mean(), cont_rec.mean(), cont_f1.mean(),  fn_geo_err.mean()*5000, fp_geo_err.mean()*5000, early_stop