...
class Renderer(object):
    ...
    def render(self, vertices, faces=None, img=None,
        ...
        #                                      c=camera_center,
        #                                      k=np.zeros(5))
        self.renderer.camera  = ProjectPoints(v=vertices, rt=ch.zeros(3), t=ch.zeros(3), 
    f=ch.array([width,width])*1.5,c=ch.array([width,height])/2., k=ch.zeros(5))
        ...
        for i, face in enumerate(faces):
            if np.any(np.isin(face, red_vertices_idx)):
                face_colors[i] = [1, 0, 0]  
